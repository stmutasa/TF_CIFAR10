...
Summary of available functions:
 ...
 To Do:
- Change Architecture. 3x3 kernels with stride 2. no pool. 256, 128, 64 consecutively
- Add a dropout layer - DONE
- Track custom Variables on Tensorboard - Training, Validation,
- Show example images on Tensorboard
- Pre-Process Inputs
...
def inference(images):
  ...
  #
  # conv1 3x3 kernels with stride 2. no pool. 512, 256, 128, 64 consecutively
  ...
  # conv2 3x3 kernels with stride 2. no pool. 256, 128, 64 consecutively
  ...
  # conv3 3x3 kernels with stride 2. no pool. 256, 128, 64 consecutively
  with tf.variable_scope('conv3') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         shape=[3, 3, 256, 512],
                                         stddev=5e-2,
                                         wd=0.0)
    conv = tf.nn.conv2d(norm2, kernel, [1, 2, 2, 1], padding='VALID')
  ...
  # conv4 3x3 kernels with stride 2. no pool. 256, 128, 64 consecutively
  with tf.variable_scope('conv4') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         shape=[3, 3, 512, 128],
                                         stddev=5e-2,
                                         wd=0.0)
    conv = tf.nn.conv2d(norm3, kernel, [1, 2, 2, 1], padding='VALID')
  ...
  #add dropout here:
  # conv4_drop = tf.nn.dropout(norm4, FLAGS.keep_prob) / FLAGS.keep_prob
